...
CREATE TABLE IF NOT EXISTS public.kpi_datapoints (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS public.kpi_objective_links (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  kpi_id UUID NOT NULL REFERENCES public.kpi_definitions(id) ON DELETE CASCADE,
  objective_id UUID NOT NULL REFERENCES public.objectives(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.alt_dashboard_preferences (
  user_id UUID PRIMARY KEY REFERENCES public.profiles(id) ON DELETE CASCADE,
...
ALTER TABLE public.alt_dashboard_preferences
  ADD COLUMN IF NOT EXISTS notes_state JSONB NOT NULL DEFAULT '{}'::jsonb;

ALTER TABLE public.alt_dashboard_preferences
  ALTER COLUMN widget_slots SET DEFAULT '["pressing", "notes", "next_due", "recent_collaborator", "key_metric"]'::jsonb;

DO $$
BEGIN
  ALTER TABLE public.alt_dashboard_preferences
    DROP CONSTRAINT IF EXISTS alt_dashboard_preferences_compute_mode_check;
  UPDATE public.alt_dashboard_preferences
    SET compute_mode = CASE
      WHEN compute_mode = 'compute' THEN 'closed'
      ELSE compute_mode
    END
    WHERE compute_mode = 'compute';
  ALTER TABLE public.alt_dashboard_preferences
    ADD CONSTRAINT alt_dashboard_preferences_compute_mode_check
    CHECK (compute_mode IN ('all', 'open', 'closed'));
END $$;

UPDATE public.alt_dashboard_preferences
SET widget_slots = jsonb_set(
  CASE
    WHEN jsonb_typeof(widget_slots) = 'array' THEN widget_slots
    ELSE '["pressing", "notes", "next_due", "recent_collaborator", "key_metric"]'::jsonb
  END,
  '{1}',
  '"notes"'::jsonb,
  true
...
CREATE TABLE IF NOT EXISTS public.alt_dashboard_presence (
  user_id UUID PRIMARY KEY REFERENCES public.profiles(id) ON DELETE CASCADE,
  last_seen_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TABLE IF NOT EXISTS public.alt_dashboard_note_folders (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.alt_dashboard_notes (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.alt_dashboard_note_attachments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_kpi_definitions_status ON public.kpi_definitions(status, category, department);
CREATE INDEX IF NOT EXISTS idx_kpi_definitions_owner ON public.kpi_definitions(owner_id);
CREATE INDEX IF NOT EXISTS idx_kpi_datapoints_kpi_period ON public.kpi_datapoints(kpi_id, period_end DESC, period_start DESC);
CREATE INDEX IF NOT EXISTS idx_kpi_datapoints_imported_by ON public.kpi_datapoints(imported_by);
CREATE INDEX IF NOT EXISTS idx_kpi_objective_links_kpi ON public.kpi_objective_links(kpi_id);
CREATE INDEX IF NOT EXISTS idx_kpi_objective_links_objective ON public.kpi_objective_links(objective_id);
CREATE INDEX IF NOT EXISTS idx_kpi_checkins_kpi ON public.kpi_checkins(kpi_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_kpi_alert_rules_kpi ON public.kpi_alert_rules(kpi_id, enabled);
CREATE INDEX IF NOT EXISTS idx_kpi_alert_events_kpi ON public.kpi_alert_events(kpi_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_kpi_import_batches_created ON public.kpi_import_batches(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_alt_dashboard_preferences_updated ON public.alt_dashboard_preferences(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_alt_dashboard_presence_seen ON public.alt_dashboard_presence(last_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_alt_note_folders_user ON public.alt_dashboard_note_folders(user_id, sort_order, name);
CREATE INDEX IF NOT EXISTS idx_alt_notes_user_edited ON public.alt_dashboard_notes(user_id, last_edited_at DESC);
CREATE INDEX IF NOT EXISTS idx_alt_notes_user_folder ON public.alt_dashboard_notes(user_id, folder_id);
CREATE INDEX IF NOT EXISTS idx_alt_notes_user_objective ON public.alt_dashboard_notes(user_id, objective_id);
CREATE INDEX IF NOT EXISTS idx_alt_notes_user_pinned ON public.alt_dashboard_notes(user_id, pinned, last_edited_at DESC);
CREATE INDEX IF NOT EXISTS idx_alt_note_attachments_user_note ON public.alt_dashboard_note_attachments(user_id, note_id);
...
DROP TRIGGER IF EXISTS set_alt_dashboard_preferences_updated_at ON public.alt_dashboard_preferences;
CREATE TRIGGER set_alt_dashboard_preferences_updated_at
  BEFORE UPDATE ON public.alt_dashboard_preferences
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

DROP TRIGGER IF EXISTS set_alt_dashboard_presence_updated_at ON public.alt_dashboard_presence;
CREATE TRIGGER set_alt_dashboard_presence_updated_at
  BEFORE UPDATE ON public.alt_dashboard_presence
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

DROP TRIGGER IF EXISTS set_alt_note_folders_updated_at ON public.alt_dashboard_note_folders;
CREATE TRIGGER set_alt_note_folders_updated_at
  BEFORE UPDATE ON public.alt_dashboard_note_folders
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

DROP TRIGGER IF EXISTS set_alt_notes_updated_at ON public.alt_dashboard_notes;
CREATE TRIGGER set_alt_notes_updated_at
  BEFORE UPDATE ON public.alt_dashboard_notes
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

DROP TRIGGER IF EXISTS set_alt_note_attachments_updated_at ON public.alt_dashboard_note_attachments;
CREATE TRIGGER set_alt_note_attachments_updated_at
...
DROP POLICY IF EXISTS "KPI definitions viewable by authenticated" ON public.kpi_definitions;
CREATE POLICY "KPI definitions viewable by authenticated"
  ON public.kpi_definitions FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "KPI owners and executives manage definitions" ON public.kpi_definitions;
...
DROP POLICY IF EXISTS "KPI datapoints viewable by authenticated" ON public.kpi_datapoints;
CREATE POLICY "KPI datapoints viewable by authenticated"
  ON public.kpi_datapoints FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "KPI datapoint actors and executives manage datapoints" ON public.kpi_datapoints;
...
DROP POLICY IF EXISTS "KPI objective links viewable by authenticated" ON public.kpi_objective_links;
...
DROP POLICY IF EXISTS "KPI owners and executives manage objective links" ON public.kpi_objective_links;
...
DROP POLICY IF EXISTS "KPI checkins viewable by authenticated" ON public.kpi_checkins;
CREATE POLICY "KPI checkins viewable by authenticated"
  ON public.kpi_checkins FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated can insert KPI checkins" ON public.kpi_checkins;
...
DROP POLICY IF EXISTS "KPI alert rules viewable by authenticated" ON public.kpi_alert_rules;
CREATE POLICY "KPI alert rules viewable by authenticated"
  ON public.kpi_alert_rules FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "KPI owners and executives manage alert rules" ON public.kpi_alert_rules;
...
DROP POLICY IF EXISTS "Authenticated can manage KPI alert events" ON public.kpi_alert_events;
...
DROP POLICY IF EXISTS "KPI import batches viewable by authenticated" ON public.kpi_import_batches;
...
DROP POLICY IF EXISTS "KPI import actors manage import batches" ON public.kpi_import_batches;
...
DROP POLICY IF EXISTS "Users upsert own alt dashboard preferences" ON public.alt_dashboard_preferences;
CREATE POLICY "Users upsert own alt dashboard preferences"
  ON public.alt_dashboard_preferences FOR ALL TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Alt dashboard presence viewable by authenticated users" ON public.alt_dashboard_presence;
...
DROP POLICY IF EXISTS "Users upsert own alt dashboard presence" ON public.alt_dashboard_presence;
CREATE POLICY "Users upsert own alt dashboard presence"
  ON public.alt_dashboard_presence FOR ALL TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users manage own alt note folders" ON public.alt_dashboard_note_folders;
CREATE POLICY "Users manage own alt note folders"
  ON public.alt_dashboard_note_folders FOR ALL TO authenticated
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users manage own alt notes" ON public.alt_dashboard_notes;
CREATE POLICY "Users manage own alt notes"
  ON public.alt_dashboard_notes FOR ALL TO authenticated
  USING ((select auth.uid()) = user_id)
  WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users manage own alt note attachments" ON public.alt_dashboard_note_attachments;
...
DROP POLICY IF EXISTS "Anyone can read profile avatar objects" ON storage.objects;
CREATE POLICY "Anyone can read profile avatar objects"
  ON storage.objects FOR SELECT TO public
  USING (bucket_id = 'profile-avatars');

DROP POLICY IF EXISTS "Users can upload own profile avatar objects" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete own profile avatar objects" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can read own alt note file objects" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can upload own alt note file objects" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can update own alt note file objects" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete own alt note file objects" ON storage.objects;
...
-- ---------------------------------------------------------------------------
-- 2026-08-19 hardening + Fix-It retirement. Historical rows and storage
-- objects remain intact, but no browser role can read, write, subscribe to,
-- or upload into the retired feature.
-- ---------------------------------------------------------------------------
...
CREATE POLICY "Users can update own avatar" ON public.profiles FOR UPDATE TO authenticated
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'fix_it_posts') THEN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.fix_it_posts;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'fix_it_comments') THEN
    ALTER PUBLICATION supabase_realtime DROP TABLE public.fix_it_comments;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'fix_it_attachments') THEN
...
BEGIN
  ...
  SELECT count(*), min(created_at) INTO used_count, oldest_event
  FROM public.api_rate_limit_events
  WHERE actor_key = p_actor_key AND scope = p_scope
    AND created_at >= NOW() - make_interval(secs => safe_window);
  IF used_count >= safe_limit THEN
    RETURN QUERY SELECT false, 0, GREATEST(1, CEIL(EXTRACT(EPOCH FROM (
      oldest_event + make_interval(secs => safe_window) - NOW()
    )))::INTEGER);
    RETURN;
  END IF;
  INSERT INTO public.api_rate_limit_events (actor_key, scope) VALUES (p_actor_key, p_scope);
...
ALTER FUNCTION public.create_default_objective_workflow() SET search_path = pg_catalog, public;
ALTER FUNCTION public.create_default_project_assessment_artifacts() SET search_path = pg_catalog, public;
...
DO $$
DECLARE policy_row RECORD; next_qual TEXT; next_check TEXT;
BEGIN
  FOR policy_row IN
    SELECT schemaname, tablename, policyname, qual, with_check FROM pg_policies
    WHERE schemaname IN ('public', 'storage') AND (
      COALESCE(qual, '') LIKE '%auth.uid()%' OR COALESCE(qual, '') LIKE '%auth.jwt()%'
      OR COALESCE(with_check, '') LIKE '%auth.uid()%' OR COALESCE(with_check, '') LIKE '%auth.jwt()%'
    )
  LOOP
    IF policy_row.qual IS NOT NULL THEN
      ...
      EXECUTE format('ALTER POLICY %I ON %I.%I USING (%s)', policy_row.policyname, policy_row.schemaname, policy_row.tablename, next_qual);
    END IF;
    IF policy_row.with_check IS NOT NULL THEN
      ...
      EXECUTE format('ALTER POLICY %I ON %I.%I WITH CHECK (%s)', policy_row.policyname, policy_row.schemaname, policy_row.tablename, next_check);
    END IF;
  END LOOP;
END
$$;
