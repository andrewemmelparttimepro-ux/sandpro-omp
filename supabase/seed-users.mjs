...
const password = process.env.SANDPRO_SEED_PASSWORD;

if (!supabaseUrl || !serviceRoleKey || !password) {
  console.error('Set SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, and SANDPRO_SEED_PASSWORD first.');
  console.error('Use staging variables for fixture seeding. Do not run broad seed scripts against production.');
  process.exit(1);
}
...
const defaultUsers = [
  { email: "jfeil@sandpro.com", name: "Jake Feil", initials: "JF", title: "CEO", department: "Leadership", role: "executive", reports_to: null, color: "#ff7f02" },
  ...
  { email: "danderson@sandpro.com", name: "Drew Anderson", initials: "DA", title: "Automation Director", department: "Automation", role: "manager", reports_to: "jfeil@sandpro.com", color: "#ff7f02" },
...
const users = process.env.SANDPRO_SEED_USERS_JSON
  ? JSON.parse(process.env.SANDPRO_SEED_USERS_JSON)
  : defaultUsers;
...
async function seed() {
  ...
  console.log('\nDone. Users created with temporary passwords; users must change password at first login.');
