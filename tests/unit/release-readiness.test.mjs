...
test('NCR tracker is a database-backed production page with objective handoff', () => {
  ...
  assert.match(pages, /departmentFilters/);
  assert.match(pages, /ncr-multi-filter/);
...
test('Vercel cron jobs are configured for digest and reminders', () => {
  ...
  const digest = config.crons.find((cron) => cron.path === '/api/cron/daily-digest');
  assert.ok(paths.includes('/api/cron/daily-digest'));
  assert.equal(digest.schedule, '0 13 * * 1-5');
...
test('mobile PWA rebuild has install assets, safe-area shell, and phone-native work surfaces', () => {
  ...
  assert.ok(manifest.icons.some(icon => icon.src === '/pwa/sandpro-omp-icon-192-v3.png' && icon.purpose.includes('maskable')));
  assert.ok(manifest.icons.some(icon => icon.src === '/pwa/sandpro-omp-icon-512-v3.png' && icon.purpose.includes('maskable')));
  assert.ok(manifest.icons.some(icon => icon.src === '/favicon-omp-v3.png' && icon.type === 'image/png'));
  assert.match(index, /viewport-fit=cover/);
  assert.match(index, /rel="icon" type="image\/png" href="\/favicon-omp-v3\.png"/);
  assert.match(index, /apple-touch-icon" href="\/pwa\/sandpro-omp-apple-touch-icon-v3\.png"/);
